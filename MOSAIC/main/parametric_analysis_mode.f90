...
subroutine parametric_analysis_mode(gam_ratio, iter_mesa, aH2O_a)
  use module_data_mosaic_kind,  only: r8
  use module_data_mosaic_main,  only: RH, pr_atm, te, cair_molm3
  ...
  !inouts
  real(r8), dimension(nbin_a_max), intent(inout) :: gam_ratio !BSINGH - This variable is probably uninitialized.

  !outs
  integer,  dimension(nbin_a_max), intent(out) :: iter_MESA
  real(r8), dimension(nbin_a_max), intent(out) :: aH2O_a

  !Local Variables
  integer jdum, idum, ibin, iter_kelvin
  real(r8) :: solvated_water_mass, water_mass, wet_mass, xt
  real(r8) :: T_K, P_atm, RH_pc
  real(r8) :: aH2O, cair_mol_m3, cair_mol_cc
