subroutine aerchemistry(it, t_out, t_in,                    & !intent-ins
  ...
  call map_mosaic_species_BOX( 0, jaerosolstate,      &          
       cnn, aer, gas, jhyst_leg, num_a, Dp_dry_a,     &
...
!    stop
  ...
  call map_mosaic_species_BOX( 1, jaerosolstate,      &          
       cnn, aer, gas, jhyst_leg, num_a, Dp_dry_a,     &
       sigmag_a, water_a, water_a_hyst, cair_mol_m3   )
  ...
  ! update: nov 2001
  !-----------------------------------------------------------------------
  subroutine map_mosaic_species_BOX( imap, jaerosolstate,  &
       cnn, aer, gas, jhyst_leg, num_a, Dp_dry_a,          &
       sigmag_a, water_a, water_a_hyst, cair_mol_m3        )
    ...
    if(imap.eq.0)then    
       ! map from host code arrays (cnn in this case) to 
       ! mosaic aerchem working arrays (gas, aer, num_a, etc)
       gas(:) = 0.0_r8
       aer(:,:,:) = 0.0_r8

       ! gases -- cnn = molecules/cm^3,   gas = nmol/m^3
       gas(ih2so4_g) = cnn(kh2so4)*conv1
       ...
       if (m_partmc_mosaic <= 0) then
        ! do this only for mosaic box-model (skip for partmc_mosaic)
          ...
          do ibin = 1, nbin_a
             ...
             num_a(ibin)      = cnn(noffset + knum_a)             ! both are #/cm^3
             Dp_dry_a(ibin)   = cnn(noffset + kdpdry_a) * 1.e-4   ! dry diameter -- cnn = um,   dp_dry_a = cm
             sigmag_a(ibin)   = cnn(noffset + ksigmag_a)          ! both are dimensionless
             water_a(ibin)    = cnn(noffset + kwater_a)           ! both are kg/m^3

             if (mhyst_method == mhyst_uporlo_waterhyst) then
                ! in this case, cnn holds water_a_hyst
                water_a_hyst(ibin) = cnn(noffset + kjhyst_a)      ! both are kg/m^3
                ! value of jhyst_leg should not matter
                jhyst_leg(ibin) = jhyst_undefined
             else
                ! in this case, cnn holds jhyst_leg, which can have three possible values
                tmpa = cnn(noffset + kjhyst_a)
                if (tmpa < -0.5) then
                   jhyst_leg(ibin) = jhyst_undefined
                else if (tmpa < 0.5) then
             ...
             do iaer = 1, naer
                ! aerosol mass components -- cnn = umol/m^3,   aer = nmol/m^3
       ...
       ! back to host code arrays (cnn in this case)
       ...
       if (m_partmc_mosaic <= 0) then
          ...
          do ibin = 1, nbin_a
             ...
             noffset = ngas_max + naer_tot*(ibin - 1)
             cnn(noffset + knum_a)    = num_a(ibin)            ! both are #/cm^3
             cnn(noffset + kdpdry_a)  = Dp_dry_a(ibin) * 1.e4  ! dry diameter -- cnn = um,   dp_dry_a = cm
             cnn(noffset + ksigmag_a) = sigmag_a(ibin)         ! both are dimensionless
             cnn(noffset + kwater_a)  = water_a(ibin)          ! both are kg/m^3

             if (mhyst_method == mhyst_uporlo_waterhyst) then
                ! in this case, cnn holds water_a_hyst
                ! value of jhyst_leg should not matter and is not mapped back
                if ( jaerosolstate(ibin) == all_solid  .or. &
                     jaerosolstate(ibin) == all_liquid .or. &
                     jaerosolstate(ibin) == mixed      ) then
                   cnn(noffset + kjhyst_a) = water_a_hyst(ibin)    ! both are kg/m^3
                else
                   cnn(noffset + kjhyst_a) = 0.0_r8
                end if
             else
                ! in this case, cnn holds jhyst_leg
                if ( jaerosolstate(ibin) == all_solid  .or. &
                     jaerosolstate(ibin) == all_liquid .or. &
                     jaerosolstate(ibin) == mixed      ) then
                   cnn(noffset + kjhyst_a) = jhyst_leg(ibin)*1.0_r8
                else
                   cnn(noffset + kjhyst_a) = jhyst_undefined*1.0_r8
                end if
             end if
             ...
             do iaer = 1, naer
                ! aerosol mass components -- cnn = umol/m^3,   aer = nmol/m^3
                !    (for oin, bc, oc, molecular weight = 1.0 so moles = grams)
                cnn(noffset+kwater_a+iaer) = aer(iaer,jtotal,ibin)*cnva2
             enddo
