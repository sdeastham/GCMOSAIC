...
contains
  !BSINGH - 05/28/2013(RCE updates)
!-----------------------------------------------------------------------
	subroutine sect_iface_allocate_memory

	use module_data_mosaic_asect, only:  maxd_acomp, maxd_asize, maxd_atype
	...
	return
	end subroutine sect_iface_allocate_memory
        !BSINGH - 05/28/2013(RCE updates ENDS)

  !-----------------------------------------------------------------------
  subroutine sectional_interface_1( dtchem, cnn_sv1, jaerosolstate, &
       mass_dry_a_bgn, mass_dry_a, dens_dry_a_bgn, dens_dry_a, water_a_hyst, &
       jaerosolstate_bgn, it_mosaic, cair_mol_m3, cair_mol_cc, mw_aer_mac, &
       dens_aer_mac, mw_comp_a)

    use module_data_mosaic_main, only: cnn, mw_air, ntot_used, pr_atm, rh, te
    use module_data_mosaic_aero, only: &
       inh4_a, iso4_a, jh2o, &
       mcoag_flag1, mmovesect_flag1, mnewnuc_flag1, &
       naer, naercomp, nbin_a_max
    ...
    real(r8), intent(in) :: dtchem,cair_mol_m3,cair_mol_cc
    real(r8), intent(inout), dimension(nbin_a_max)  :: mass_dry_a_bgn,mass_dry_a,dens_dry_a_bgn,dens_dry_a,water_a_hyst
    real(r8), intent(inout), dimension(naer) :: mw_aer_mac,dens_aer_mac
    real(r8), intent(inout), dimension(naercomp) :: mw_comp_a
    real(r8), intent(in) :: cnn_sv1(ntot_used)
    ...
    call map_mosaic_cnn_rbox( 0, rbox0, &
         ...
         jaerosolstate,mass_dry_a_bgn,mass_dry_a,dens_dry_a_bgn, &
         dens_dry_a,water_a_hyst,jaerosolstate_bgn,it_mosaic,    &
    ...
    if (method_movesect < 50) then
       ...
       call move_sections_x3( 1, 1, 1, 1, 1, rbox,   &
            ...
            drymass_aftgrow, drymass_pregrow,   &
            adrydens_box, awetdens_box, adrydpav_box, awetdpav_box,   &
            adryqmas_box,it_mosaic,mmovesect_flag1)
    else
    ...
    if (idiagcc > 0) &
         ...
         fact_apmassmr, fact_apnumbmr, fact_apdens, fact_apdiam, &
         jaerosolstate,jaerosolstate_bgn,it_mosaic,cair_mol_m3, &
         cair_mol_cc,mw_aer_mac,mw_comp_a)
  ...
  subroutine map_mosaic_cnn_rbox( iflagaa, rbox, &
    ...
    use module_data_mosaic_main, only: &
       ...
       lun_sect_172, mw_air, naer_tot, ngas_max, ntot_used
    use module_data_mosaic_aero, only: &
       jhyst_lo, jhyst_up, &
    ...
    ! subr parameters
    integer, intent(in) :: iflagaa,it_mosaic
    integer, intent(in), dimension(nbin_a_max) :: jaerosolstate,jaerosolstate_bgn
    ...
    if ((it_mosaic == 1) .and. (iflagaa == 0)) then
       call test_move_sections( 1, 1, 1, 1, 1, it_mosaic,mmovesect_flag1 )
    end if
    ...
    do ibin = 1, nbin_a
       ...
       if (mhyst_method == mhyst_uporlo_waterhyst) then
          ...
          ! cnn holds jhyst_leg
          ! rbox holds jhyst_leg*volume_weighted_hygroscopicity (see below)
          ...
          if ( jaerosolstate(ibin) == all_solid  .or. &
             ...
             rbox(kjhyst_a +noffset) = max( 0.0_r8, cnn(kjhyst_a +noffset) ) * tmph
          else
             rbox(kjhyst_a +noffset) = 0.0_r8
          end if
       else
          write(*,'(/a,i10/)') &
               '*** map_mosaic_cnn_rbox - bad mhyst_method =', mhyst_method
          stop
       end if
...
20000 continue
    ...
    do ibin = 1, nbin_a
       ...
       if (mhyst_method == mhyst_uporlo_waterhyst) then
          ! cnn and rbox hold water_a_hyst (in water conc units)
          cnn(kjhyst_a+noffset) = rbox(ksigmag_a+noffset)/conv_watr
       else if (mhyst_method == mhyst_force_up) then
          cnn(kjhyst_a +noffset) = jhyst_up
       else if (mhyst_method == mhyst_force_lo) then
          cnn(kjhyst_a +noffset) = jhyst_lo
       else if (mhyst_method == mhyst_uporlo_jhyst) then
          !BSINGH - 05/28/2013(RCE updatesENDS)
  ...
  subroutine mbox_sectional_diagnostics( cnn_sv1, cnn_sv2, rbox_sv, &
       ...
       awetdpav_box_sv, adryqmas_box_sv,   &
       fact_apmassmr, fact_apnumbmr, fact_apdens, fact_apdiam, &
       jaerosolstate,jaerosolstate_bgn,it_mosaic,&
    ...
    use module_data_mosaic_main, only: &
       cnn, dt_sec, &
       kdpdry_a, kjhyst_a, knum_a, kwater_a, lun_sect_172, &
       mw_air, naer_tot, ngas_max, ntot_used,&
    ...
    if (it_mosaic == 1) then
       ...
       write(lunaa,'(a)') ' '
       write(lunaa,'(a)') 'gas       units:     cnn = molec/cm^3     rbox = mol/mol'
       write(lunaa,'(a)') 'aer numb  units:     cnn = #/cm^3         rbox = #/mol'
       write(lunaa,'(a)') 'aer dpdry units:     cnn = um             rbox = cm'
       write(lunaa,'(a)') 'aer water units:     cnn = kg/m^3         rbox = mol/mol'
       write(lunaa,'(a)') 'aer mass  units:     cnn = umol/m^3       rbox = mol/mol'
       write(lunaa,'(a)') '    (note mw=1.0 for oin, bc, oc)'
    end if
