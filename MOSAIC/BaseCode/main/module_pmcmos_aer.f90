...
!-----------------------------------------------------------------------
	subroutine pmcmos_init_aerosol(mw_aer_mac,dens_aer_mac,kappa_aer_mac)
!
...
!   subr arguments 
        real(r8), intent(inout), dimension(naer) :: mw_aer_mac,dens_aer_mac,kappa_aer_mac
!   local variables
...
!-----------------------------------------------------------------------
	subroutine pmcmos_print(it,jaerosolstate)
!
!   allocates and loads "cnn" arrays for background and emissions
!      size distributions
!   loads the main mosaic cnn array with initial size distribution
...
!
	use module_data_mosaic_main, only:  &
	    cair_mlc, cair_molm3, cnn, inputfile, iprint, knum_a, kjhyst_a, kwater_a,  &
	    kso4_a, kno3_a, kcl_a, knh4_a, koc_a, kbc_a, karo1_a, kapi1_a, klim2_a,  &
	    lun_sect_183, lun_sect_184, lun_sect_185, lun_sect_186,  &
	    lun_sect_188, lun_fullout,  &
            m_partmc_mosaic, naer_tot, ngas_max, ntot_max,  & !BSINGH - 05/28/2013(RCE updates)
	    pi, ppb, pr_atm, rh, species, te, time_sec
	use module_data_mosaic_aero, only:  &
	    aer_name, gas_name, &
	    method_bcfrac, method_kappa, &
	    msectional, msectional_flag2, msize_framework, &
	    naer, nbin_a, nbin_a_max,mw_aer_mac,dens_aer_mac
	use module_data_mosaic_asect, only:  &
...
!   local variables
	...
	type( aer_dist_t ) :: ad
        ...
        tmp_n = 1.0 !BSINGH - Temporarilry initialized, please modify if required 

        !BSINGH - 05/28/2013(RCE updates)
        ! the module_data_mosaic_asect arrays, which are used in this routine,
        ! are only allocated and available for mosaic box-model
...
!	if (time_sec <= 1.0e-10) then
	...
	if (ltmp == 3 .and. it == 0) then
	    ...
	    write(lun,'(14x,18a/(21a))') ('  "', aer_name(i), '"  ', i=1,19), '  "WATER   "'!BSINGH - 05/28/2013(RCE updates)
        end if
	...
	lun = lun_sect_185
		write(lun,'(1p,7e14.6)') &!BSINGH - 05/28/2013(RCE updates)
...
! fullout output
	...
	if (it == 0) then
	    write(lun,'(2a)') &
	    'units:     gases = ppbv     num = #/cm^3     ', &
	    'dpdry = um     aero species & water = ug/m^3'
	    write(lun,'(a)') '*** only non-zero values are shown ***'
	end if

	write(lun,'(/i8,1p,5e12.4,5x,a)') &
	    it, time_sec, te, pr_atm, cair_molm3, rh, &
	    'it, time_sec, te(k), pr_atm, cair_molm3, rh(%)'

	if ( 1 == 0 ) then  ! this code section is turned off

	write(lun,'(i5,5x,a)') ngas_max, 'gases (ppbv)'
	write(lun,'(6(2x,a))') species(1:ngas_max)(1:10)
	tmpa = ppb/cair_mlc
	write(lun,'(1p,6e12.4)') cnn(1:ngas_max)*tmpa

	write(lun,'(2i5,5x,a)') nbin_a, naer_tot, 'aerosol bins, species:  xxx'
	write(lun,'(6(4x,a))') &
	    'num     ', 'dpdry   ', 'sigmag  ', &
            'jhyst   ', 'water   ', aer_name(1:naer)
	noffseta = ngas_max
        do i = 1, nbin_a
	    write(lun,'(i12,a)') i
	    write(lun,'(1p,6e12.4)') &
		cnn(noffseta+1:noffseta+5), &
		(cnn(noffseta+5+j), j=1,naer)
	    noffseta = noffseta + naer_tot
	end do
	...
	tmpa = ppb/cair_mlc
	do l = 1, ngas_max
	    if (ioptaa_fullout > 0 .and. cnn(l)*tmpa <= 0.0_r8) cycle
	    write(lun,'(i8,1p,e12.4,2x,a)') it, cnn(l)*tmpa, species(l)(1:12)
	end do
	noffseta = ngas_max
	do ibin = 1, nbin_a
	    write(lun,'(i8,i12,2x,a,i3.3)') it, jaerosolstate(ibin), 'jaerostate_a', mod(ibin,1000)
	    do j = 1, 5
		l = noffseta + j
		tmpa = 1.0_r8
		if (j == 5) then
		    tmpa = 1.0e9_r8  ! water -- convert kg/m^3 to ug/m^3
		end if
		if (ioptaa_fullout > 0 .and. cnn(l)*tmpa <= 0.0_r8 .and. j /= kjhyst_a) cycle
		write(lun,'(i8,1p,e12.4,2x,a)') it, cnn(l)*tmpa, species(l)(1:12)
	    end do
	    do j = 1, naer
		l = noffseta + 5 + j
		tmpa = mw_aer_mac(j)  ! so4, no3, etc -- convert umol/m^3 to ug/m^3
!		tmpa = 1.0_r8
		if (ioptaa_fullout > 0 .and. cnn(l)*tmpa <= 0.0_r8) cycle
		write(lun,'(i8,1p,e12.4,2x,a)') it, cnn(l)*tmpa, species(l)(1:12)
	    end do
