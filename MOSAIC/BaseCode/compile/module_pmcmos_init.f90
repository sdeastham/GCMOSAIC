...
contains
  ...
  subroutine pmcmos_init
    ...
    use module_data_mosaic_main, only: cnn, lun_sect_190, ngas_max, pblh, pr_atm, &
  ...
  subroutine pmcmos_read_inputs
    ...
    if (idiagcc > 0) then
       ...
       write(*,'(a,1p,10e10.2)' ) 'rate adjust ', aer_emit_adjust_fac!BSINGH - 05/28/2013(RCE updates)
       write(*,'(a,1p,10(1x,a))') 'dist_fname  ', ( trim(aer_emit_dist(i)%dist_fname), i=1,aer_emit_nt )
    end if ! (idiagbb > 0)
    
    aer_emit_fac(1:aer_emit_nt) = aer_emit_fac(1:aer_emit_nt)*aer_emit_adjust_fac!BSINGH - 05/28/2013(RCE updates)
  ...
  subroutine pmcmos_readfile_aa( idiagaa, iok, &
    ...
    character(len=640) :: tmp_fname
    
    if (pmcmos_inputfile_directory == ' ') then
       tmp_fname = fname
    else
       tmp_fname = trim(pmcmos_inputfile_directory) // fname
    end if
    
    open( unit=lun, file=tmp_fname, status='old', iostat=itmpa )
...
7200 write(*,'(2a)') '*** pmcmos_readfile_aa error = ', trim(txtee)
 	write(*,'(2a)') 'filename = ', trim(tmp_fname)!BSINGH - 05/28/2013(RCE updates)
 stop
...
subroutine pmcmos_read_aer_distribs
 ...
 use module_data_mosaic_aero, only: aer_name, &
      !BSINGH - 05/28/2013(RCE updates)
      iso4_a,  ino3_a,  icl_a,   inh4_a,  imsa_a, &
      jnh4hso4,jnh4no3, jnacl,            jmsa, &
      iaro1_a, iaro2_a, ialk1_a, iole1_a, &
      jaro1,   jaro2,   jalk1,   jole1, &
      iapi1_a, iapi2_a, ilim1_a, ilim2_a, &
      japi1,   japi2,   jlim1,   jlim2, &
      ico3_a,  ina_a,   ica_a,   ioin_a,  ioc_a,   ibc_a, &
      jcaco3,           jcaco3,  join,    joc,     jbc, &
      naer, kappa_aer_mac, mw_comp_a, mw_aer_mac, dens_comp_a, &
      dens_aer_mac
 ...
 !BSINGH - 05/28/2013(RCE updates)
 write(*,'(/a/2a)') 'subr pmcmos_read_aer_distribs', &
      'iaer, jcomp, aer_name,   dens_aer_mac, dens_comp_a,   ', &
      'mw_aer_mac, mw_comp_a,   kappa_aer_mac'
 do i = 1, naer
    j = -999888777
    if (i == iso4_a ) j = jnh4hso4
    if (i == ino3_a ) j = jnh4no3
    if (i == icl_a  ) j = jnacl
    if (i == inh4_a ) j = jnh4hso4
    if (i == imsa_a ) j = jmsa
    if (i == iaro1_a) j = jaro1
    if (i == iaro2_a) j = jaro2
    if (i == ialk1_a) j = jalk1
    if (i == iole1_a) j = jole1
    if (i == iapi1_a) j = japi1
    if (i == iapi2_a) j = japi2
    if (i == ilim1_a) j = jlim1
    if (i == ilim2_a) j = jlim2
    if (i == ico3_a ) j = jcaco3
    if (i == ina_a  ) j = jnacl
    if (i == ica_a  ) j = jcaco3
    if (i == ioin_a ) j = join
    if (i == ioc_a  ) j = joc
    if (i == ibc_a  ) j = jbc
    write(*,'(2i4,2x,a,2x,2f8.4,2x,2f8.2,f10.4)') &
         i, j, aer_name(i), dens_aer_mac(i), dens_comp_a(j), &
         mw_aer_mac(i), mw_comp_a(j), kappa_aer_mac(i)
 end do
 !BSINGH - 05/28/2013(RCE updates ENDS)
...
!-----------------------------------------------------------------------
subroutine pmcmos_read_1_aer_distrib( idiagaa_inp, iok, &!BSINGH - 05/28/2013(RCE updates)
    lun, ad )
  !
  !   reads data from a single aerosol (size) distribution file (back, emit, or init)!BSINGH - 05/28/2013(RCE updates)
  !

  !   subr arguments (none)
 integer, intent(in)  :: idiagaa_inp, lun!BSINGH - 05/28/2013(RCE updates)
 integer, intent(out) :: iok
 type( aer_dist_t ), intent(inout) :: ad

 !   local variables
 integer, parameter :: maxch = 128, maxlnlen=2000
 integer :: i, idiagaa, iline, imode, imodeb, itmpa!BSINGH - 05/28/2013(RCE updates)
 integer :: ipostn(ntmax_pmcmos), itype(ntmax_pmcmos), ival(ntmax_pmcmos)
 integer :: j, jp
 integer :: nchqq(ntmax_pmcmos)
 integer :: np, ntok!BSINGH - 05/28/2013(RCE updates)
 ...
 if (pmcmos_inputfile_directory == ' ') then
    tmp_fname = ad%dist_fname
 else
    tmp_fname = trim(pmcmos_inputfile_directory) // ad%dist_fname
 end if
 ...
 if (pmcmos_inputfile_version >= 2) then
    np = 5
    str_expected(1) = 'mass_frac         '
    str_expected(2) = 'mode_type         '
    str_expected(3) = 'num_conc          '
    str_expected(4) = 'geom_mean_diam    '
    str_expected(5) = 'log10_geom_std_dev'
 end if
 !BSINGH - 05/28/2013(RCE updates ENDS)    
 ad%n_mode = 0
 imodeb = 0
 ...
 do jp = 1, np!BSINGH - 05/28/2013(RCE updates)
    iline = iline + 1
    if (idiagaa > 0) write(*,'(i5,2x,a)') iline, trim(ad%dist_fname)
    read(lun,'(a)',end=7100) ast
    if (idiagaa > 0) write(*,'(3a)') 'ast = |', trim(ast), '|'
    ...
    if (pmcmos_inputfile_version <= 1) then!BSINGH - 05/28/2013(RCE updates)
       if      (jp == 1) then
...
subroutine pmcmos_read_1_ad_comps( idiagaa, iok, &
 jj, ad )
  !
  !   reads data from a single aerosol composition file (back, emit, or init)!BSINGH - 05/28/2013(RCE updates) 
  !
