...
!-----------------------------------------------------------------------
	...
	use module_data_mosaic_main, only:  pi, cair_molm3!BSINGH - 05/28/2013(RCE updates)
	use module_peg_util
...
!BSINGH - 05/28/2013(RCE updates ENDS)
	use module_data_mosaic_main, only:  kh2so4, knh3, kso2, ntot_used, piover6, third
...
!	    rbox(l) = rbox(l) + qso4a_del
	    ...
	    rbox(l) = rbox(l) + tmpa/fact_apmassmr
            !BSINGH - 05/28/2013(RCE updates)
            tmpvecf(2) = rbox(l)
            ! (whatever to g/g-air) * (then ug/g-air) * (then ug/m3-air)
	    tmpvecf(1:2) = tmpvecf(1:2) * fact_apmassmr         * 1.0e6           * (cair_molm3*mw_air)
...
!	rbox(l) = rbox(l) + qnuma_del
	...
	rbox(l) = rbox(l) + tmpa/fact_apnumbmr
        !BSINGH - 05/28/2013(RCE updates)
        tmpvecf(2) = rbox(l)
        ! (whatever to #/g-air) * (then #/m3-air)      * (then #/cm3-air)
	tmpvecf(1:2) = tmpvecf(1:2) * fact_apnumbmr         * (cair_molm3*mw_air) * 1.0e-6
	tmpvecf(3) = tmpvecf(2) - tmpvecf(1)
	if (idiagbb >= 10) &
