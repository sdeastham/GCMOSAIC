	subroutine miedriver(dp_wet_a,dp_core_a,ri_shell_a,ri_core_a,vlambc,qextc,qscatc,gscac,extc,scatc)
! MOSAIC INPUTS
!	dp_wet_a = diameter (cm) of aerosol
!	dp_core_a = diameter (cm) of the aerosol's core
!	ri_shell_a = refractive index (complex) of shell
!	ri_core_a = refractve index (complex ) of core (usually assumed to be LAC: Light Absorbing Carbon)
...
! /*---------------------------------------------------------------*/
      ...
      complex*8 ri_shell_a,ri_core_a
!
	nang=2 ! only one angle
	nrgflagc=0 ! size distribution
!
	rgc=(dp_wet_a/2.0)*1.0d4 ! radius of particle, convert from cm to microns
	...
	coreic=aimag(ri_core_a)
       CALL ACKMIEPARTICLE( VLAMBc,NRGFLAGc,RGcmin,RGcmax, &
	       ...
	       extc=extc*1.0e-08 ! convert um^2 to cm^2
	       scatc=scatc*1.0e-08 ! convert um^2 to cm^2
...
!     PARAMETER(MXNANG=501, MXNWORK=500000) ! old values JCB
      PARAMETER(MXNANG=501, MXNWORK=10000)  ! new values JCB
